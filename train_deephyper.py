    ...
    for dataset_name in ['dogs']:
        ...
        for K in [2,4,8]:
            ...
            for method in ['erm','jtt', 'suby', 'subg', 'rwy', 'rwg', 'dro']:
                ...
                args.group_best = f"NOHBO_{args.SMA.name}_K={args.SMA.K}_{args.method}_mu={args.SMA.mu}"
                ...
                for i in range(args.n_eval_init_seed)[::-1]:  # -1 to have reverse order to 0 in the end for next outer loop
                    args["init_seed"] = i
                    run()
